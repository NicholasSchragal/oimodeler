...
def fastRotator(dim0, size, incl, rot, Tpole, lam, beta=0.25):
    ...
    a = 2.0 / 3 * (rot) ** 0.4 + 1e-9
    K = np.sin(1.0 / 3.0) * np.pi
    ...
    r0 = a * np.sin(1 / 3.0) * np.arcsin(x0) / (1.0 / 3.0 * x0)

    x2 = (1.5 * a) ** 1.5 * np.sin(theta)
    rin = a * np.sin(1 / 3.0) * np.arcsin(x2) / (1.0 / 3.0 * x2)
    ...
    if nlam == 1:
        flx = 1.0 / (np.exp(K1 / (lam * Teff)) - 1)

        im = np.zeros([dim, dim])

        for iz in range(dim):
            im = (
                im * (im != 0) + (im == 0) * dr[:, :, iz] * flx[:, :, iz]
            )  # *limb[:,:,iz]
        ...
        flx = 1.0 / (np.exp(K1 / np.einsum("ijk, l->ijkl", Teff, lam)) - 1)

        im = np.zeros([dim, dim, nlam])

        for iz in range(dim):
            im = im * (im != 0) + dr[:, :, iz, :] * flx[:, :, iz, :] * (
                im == 0
            )
...
class oimFastRotator(oim.oimComponentImage):
    ...
    def __init__(self, **kwargs):
        ...
        # NOTE: Component parameters. Note that as it inherits from the oimComponentImage
        # class it already has x, y, f and dim as parameters
        ...
        self.params["beta"] = oim.oimParam(
            name="beta",
            value=0.25,
            description="Gravity Darkening Exponent",
            unit=units.one,
        )
    ...
    def _internalImage(self):
        ...
        # NOTE: Compute the pixelSize based on the internal image size and the polar diameter
...
for iwl in range(nwl):
    cwl = iwl / (nwl - 1)
    ax[0].plot(
        B / wl[iwl] / units.rad.to(units.mas),
        v[iwl, : nB // 2],
...
for i in range(2):
    ...
    ax[i].set_xlabel(r"B/$\lambda$ (cycles/rad)")
...
# NOTE: Modify the fast rotator parameters and  creating a new model with UD component
c.params["f"].value = 0.9
c.params["pa"].value = 30
ud = oim.oimUD(d=1, f=0.1, y=10)
m2 = oim.oimModel(c, ud)

# NOTE: Plot the new model image
m2.showModel(
    512,
    0.06,
    wl=[1e-6, 10e-6],
...
# NOTE:  Plot an image and the visibility side by side ( for main page of documentation)
fig, ax = plt.subplot_mosaic(
    [["upper left", "upper right"], ["lower", "lower"]], figsize=(8, 9)
)
...
ax[2].set_xlabel(r"B/$\lambda$ (cycles/rad)")
ax[2].set_ylabel("Visibility")
ax[2].legend()

plt.subplots_adjust(
    left=0.10, bottom=0.15, right=0.95, top=0.95, wspace=0.25, hspace=0.25
)
